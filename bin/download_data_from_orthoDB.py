...
def get_sequences(orig_clusters_comb, data):
    ...
    command2= ["curl", "-s","--resolve","data.orthodb.org:129.194.190.40", "https://data.orthodb.org/current/fasta?id=%s&species="%(data), "-L", '-o', outfname]
    response2 = subprocess.Popen(command2)
    out2, err2 = response2.communicate(timeout=20)
    ...
    for retry in range(max_retries):
        try:
            ...
            with open(map_log_file, 'a') as logger:
                ...
                logger.write("current_dir: " + current_dir) 
                response = subprocess.Popen(command1, stdout=subprocess.PIPE) # pipe the terminal output to response
                out, err = response.communicate(timeout=20) # Wait for process to terminate
            ...
            #print(out)
                ...
                try:
                    ...
                    print("map_retrieve parallize jobs call error: " + str(err))
...
if __name__ == "__main__":
    if len(sys.argv) < 1:
        ...
        with open(log_file, 'a') as logger:
            ...
            single_cluster = map_retrieve(ncbi_id=ncbi_id, w_dir=d_dir, orig_clusters_comb=orig_clusters_comb,max_retries=3,log_dir=tax_rank['log_dir'])
